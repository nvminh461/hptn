...
form.addEventListener("submit", (e) => {
    e.preventDefault();
    const errorMessages = checkInputs();
    if (!errorMessages.some(message => message !== "")) {
        // No errors, display alert with entered information
        const enteredInfo = {
            classId: classId.value.trim(),
            className: className.value.trim(),
            khoaHoc: khoaHoc.value.trim(),
            gvcn: gvcn.value.trim(),
        };
        alert(JSON.stringify(enteredInfo, null, 2));
    }
});
